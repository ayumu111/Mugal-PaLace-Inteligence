...
class MyEval_8128_shimayuu {
    ...
    public float value(Board board) {
        ...
        if (piecesCount > 50) {
            // 終盤は石数評価を重視し、全探索を促す
            return board.score() * 15000;
        } else if (piecesCount > 24) {
            return board.score() * 200;
        }
        ...
        List<Move> blackMoves = board.findLegalMoves(Color.BLACK);
        List<Move> whiteMoves = board.findLegalMoves(Color.WHITE);
        mobilityBonus += (blackMoves.size() - whiteMoves.size()) * 10;

        return eval + mobilityBonus;
    }
...
public class MyPlayer extends ap25.Player {
    static final String MY_NAME = "shim";
    MyEval_8128_shimayuu eval;
    int depthLimit;
    Move move;
    OurBoard_shimayuu board;

    public MyPlayer(Color color) {
        this(MY_NAME, color, new MyEval_8128_shimayuu(), 8);
    }

    public MyPlayer(String name, Color color, MyEval_8128_shimayuu eval, int depthLimit) {
        super(name, color);
        this.eval = eval;
        this.depthLimit = depthLimit;
        this.board = new OurBoard_shimayuu();
    }

    public MyPlayer(String name, Color color, int depthLimit) {
        this(name, color, new MyEval_8128_shimayuu(), depthLimit);
    }
    ...
    public Move think(Board board) {
        ...
        if (legalMoves.isEmpty()) {
            ...
            float evalValue = maxSearch(newBoard, Float.NEGATIVE_INFINITY, Float.POSITIVE_INFINITY, 0);
            // System.out.printf("評価値: %.2f%n", evalValue);
            ...
            if (this.move.isPass()) {
                ...
                int x = this.move.getIndex() % SIZE;
                int y = this.move.getIndex() / SIZE;
                // System.out.printf("(%d, %d)に配置%n", x, y);
            }
        }
    ...
    float maxSearch(OurBoard_shimayuu board, float alpha, float beta, int depth) {
    ...
    float minSearch(OurBoard_shimayuu board, float alpha, float beta, int depth) {
        if (isTerminal(board, depth))
            return this.eval.value(board);
    ...
    boolean isTerminal(Board board, int depth) {
        ...
        // 終盤は探索深さを大きくする
        int dynamicDepthLimit = piecesCount > 50 ? 25 : (piecesCount > 24 ? this.depthLimit + 5 : this.depthLimit);
        return board.isEnd() || depth > dynamicDepthLimit;
    }

    List<Move> order(List<Move> moves, OurBoard_shimayuu board, Color color) {
        // 評価値の高い順にソート
        moves.sort((a, b) -> {
            ...
            // 角を優先
            int[] corners = { 0, SIZE - 1, LENGTH - SIZE, LENGTH - 1 };
            boolean aIsCorner = IntStream.of(corners).anyMatch(i -> i == a.getIndex());
            boolean bIsCorner = IntStream.of(corners).anyMatch(i -> i == b.getIndex());
...
// --- ここからビットボード実装 ---
class OurBoard_shimayuu implements ap25.Board, Cloneable {
    private long black;
    private long white;
    ...
    public OurBoard_shimayuu(long black, long white, long block, int lastMove, Color lastColor) {
        this.black = black;
        this.white = white;
        this.block = block;
    ...
    @Override
    public OurBoard_shimayuu clone() {
        return new OurBoard_shimayuu(black, white, block, lastMove, lastColor);
    }
    ...
    @Override
    public OurBoard_shimayuu placed(Move move) {
        if (move.isPass()) {
            return new OurBoard_shimayuu(black, white, block, -1, move.getColor());
        }
        ...
        // ブロック上には置けない
        if ((bl & mask) != 0) {
            return new OurBoard_shimayuu(b, w, bl, lastMove, lastColor); // 何も変化しない
        }
        me |= mask;
        for (int d = 0; d < 8; d++) {
        ...
        if (color == Color.BLACK) {
            return new OurBoard_shimayuu(me, opp, bl, idx, color);
        } else {
            return new OurBoard_shimayuu(opp, me, bl, idx, color);
        }
    }

    @Override
    public OurBoard_shimayuu flipped() {
        return new OurBoard_shimayuu(white, black, block, lastMove,
                lastColor == Color.BLACK ? Color.WHITE : Color.BLACK);
    }
